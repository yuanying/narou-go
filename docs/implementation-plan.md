...
| 状態 | フェーズ | 対象 | 完了条件 | 検証 |
|------|----------|------|----------|------|
| ⏳ 次に着手 | Library Reader | `internal/library` | `database.yaml`、`toc.yaml`、`本文/*.yaml` を fixture から読み込める | `go test ./...` |
| 未着手 | HTML Converter | `internal/converter` | 本文 HTML を EPUB 用 XHTML に変換できる | `go test ./...` |
